...
/* =========================================================
   COUNTRY UI & RESPONSIVE GRID FIXES
========================================================= */
...
function setupCountrySystem() {
    ...
    wrapper.innerHTML = `
        <div class="bg-surface-container-lowest rounded-2xl border border-surface-container p-4 shadow-sm">
            ...
            <div id="cdPopularCountries" class="flex flex-wrap sm:flex-nowrap gap-2 overflow-x-auto pb-1"></div>
            ...
            <button id="cdMoreCountriesBtn" type="button" class="mt-3 text-[12px] font-semibold text-primary hover:underline">
                More Countries
            </button>
        </div>
...
function createCountryButton(country) {
    ...
    button.className = "flex-shrink-0 min-w-[48px] h-10 px-2 rounded-xl bg-surface-container-low hover:bg-surface-container border border-transparent flex items-center justify-center text-[22px] transition-all";
    button.title = country.name;
    button.innerHTML = country.flag;

    if (country.code === currentCountry) {
        button.className = "flex-shrink-0 min-w-[48px] h-10 px-2 rounded-xl bg-primary-container text-white border-2 border-primary flex items-center justify-center text-[22px] shadow-sm";
    }
...
async function fetchDealsAndInit() {
    try {
        ...
        globalDeals = Array.isArray(data.deals) ? data.deals : [];
    } catch (error) {
...
async function fetchCountryDeals() {
    ...
    try {
        const response = await fetch(`${API_BASE}/api/deals?country=${encodeURIComponent(countryCode)}`);
        if (response.ok) {
            const data = await response.json();
            if (data.success && Array.isArray(data.deals)) {
                const apiDeals = data.deals.map(deal => normalizeDeal(deal));
                const filtered = apiDeals.filter(deal => getDealCountry(deal) === countryCode);
        ...
        console.warn("Country API failed. Using local filtering:", error);
    }
...
/* =========================================================
   DEAL NORMALIZATION & SORTING (FEATURED STICKY LOGIC)
========================================================= */
...
function normalizeDeal(deal) {
    ...
    copy.is_featured = (Number(deal.is_featured) === 1 || deal.isFeatured === true) ? 1 : 0;
    return copy;
}
...
function getDealCountry(deal) {
    ...
    const nameMap = {
        ...
        "UNITED KINGDOM": "GB", "UK": "GB", "OMAN": "OM",
...
function renderCountryCategories() {
    ...
    const categories = [...new Set(globalDeals.map(deal => String(deal.category || "").trim()).filter(Boolean))];

    let html = `<div class="flex items-center gap-2 overflow-x-auto pb-1"><button type="button" data-category="all" class="cd-category-btn flex-shrink-0 px-3.5 py-1.5 rounded-xl font-label-md text-[13px]">All (${globalDeals.length})</button>`;

    categories.forEach(category => {
        const count = globalDeals.filter(deal => String(deal.category || "").toLowerCase() === category.toLowerCase()).length;
...
/* =========================================================
   SPOTLIGHT (REAL RATING & SAVINGS BADGE)
========================================================= */
...
function loadSpotlight() {
    ...
    // Sticky Featured selection: explicitly pick featured deal or fallback to first
    let featuredDeal = deals.find(d => Number(d.is_featured) === 1);
    if (!featuredDeal) featuredDeal = deals[0];
...
/* =========================================================
   DEAL CARDS (SORTED WITH STICKY FEATURED ON TOP & SAVINGS BADGE)
========================================================= */
...
function loadDeals(filter = "all") {
    ...
    let deals = globalDeals.filter(deal => getDealCountry(deal) === currentCountry);

    if (filter !== "all") {
        deals = deals.filter(deal => String(deal.category || "").toLowerCase() === String(filter).toLowerCase());
    }

    // Sort: Featured deal strictly on top, then higher discount/recent
    deals.sort((a, b) => {
    ...
    if (deals.length === 0) {
        ...
        container.innerHTML = `
            <div class="p-6 text-center text-on-surface-variant bg-surface-container-lowest rounded-2xl border border-surface-container">
                ...
                <div class="text-[12px]">There are currently no deals listed for ${country ? escapeHtml(country.name) : "this country"}.</div>
            </div>
        `;
        return;
    }
    ...
    deals.forEach(deal => {
        ...
        const savingsAmount = (oldPrice && oldPrice > newPrice) ? oldPrice - newPrice : 0;
        const dealId = deal.id || "";
        const storeName = String(deal.store || deal.store_name || deal.storeName || deal.retailer || deal.retailer_name || "Store").trim();
        const isFeatured = Number(deal.is_featured) === 1;

        const card = document.createElement("div");
        card.className = `bg-surface-container-lowest rounded-2xl p-4 shadow-sm border ${isFeatured ? 'border-primary ring-1 ring-primary/30' : 'border-surface-container/60'} flex flex-col gap-3`;

        card.innerHTML = `
            <div class="flex items-center justify-between gap-2">
                <div class="flex items-center gap-1.5 min-w-0">
                    <span class="text-[20px] flex-shrink-0">${getDealFlag(deal)}</span>
                    ...
                    <span class="text-on-surface-variant text-[11px] flex-shrink-0">·</span>
                    <span class="font-semibold text-[12px] text-on-surface truncate" title="${escapeAttribute(storeName)}">
                        ${escapeHtml(storeName)}
                    </span>
                    ${isFeatured ? '<span class="ml-1 px-2 py-0.5 rounded bg-primary-container text-on-primary text-[9px] font-bold uppercase">Featured</span>' : ''}
                </div>
            ...
            <div class="flex gap-3">
                ...
                <div class="flex flex-col min-w-0 justify-center flex-1">
                    <div class="flex items-center gap-1 mb-1">
                        ${deal.category ? `<span class="text-[10px] font-semibold text-primary">${escapeHtml(deal.category)}</span>` : ""}
                    </div>
                    ...
                    ${savingsAmount > 0 ? `
                        <div class="mt-1 inline-flex items-center gap-1 text-[11px] font-bold text-savings-green bg-savings-green-subtle px-2 py-0.5 rounded-md w-fit">
            ...
            <div class="flex items-center justify-between pt-2 border-t border-surface-container-low text-[11px] text-on-surface-variant">
                <div class="flex items-center gap-1 min-w-0">
                    ...
                    <span class="truncate">${escapeHtml(deal.last_verified_at || deal.created_at || "Verified")}</span>
                </div>
                <div class="flex items-center gap-2 flex-shrink-0">
                    ${dealId ? `
                        <button onclick="openDealComparison('${escapeAttribute(String(dealId))}')" class="py-2 px-2.5 rounded-xl bg-surface-container-low hover:bg-surface-container text-on-surface font-semibold flex items-center gap-1" title="Compare Prices">
                            <span class="material-symbols-outlined text-[17px]">compare_arrows</span>
                        </button>
                    ` : ""}
                    <button onclick="shareDeal('${encodeURIComponent(deal.title || "")}', '${escapeAttribute(deal.url || window.location.href)}')" class="w-9 h-9 rounded-xl bg-surface-container-low hover:bg-surface-container text-on-surface flex items-center justify-center transition-colors shadow-sm" title="Share Deal">
                        <span class="material-symbols-outlined text-[18px]">share</span>
                    </button>
                    <a href="${escapeAttribute(deal.url || "#")}" target="_blank" rel="noopener noreferrer" class="py-2 px-3.5 rounded-xl bg-primary-container hover:bg-primary text-on-primary font-label-md text-[13px] font-semibold flex items-center gap-1 shadow-sm">
                        <span>Get Deal</span>
                        <span class="material-symbols-outlined text-[16px]">arrow_forward</span>
                    </a>
                </div>
...
async function openDealComparison(dealId) {
        ...
        modal.innerHTML = `
            <div class="w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-surface-container-lowest rounded-2xl shadow-2xl">
                <div class="sticky top-0 bg-surface-container-lowest border-b border-surface-container p-4 flex items-center justify-between">
                    <div>
                        <div class="text-[11px] font-bold uppercase tracking-wider text-primary">Price Comparison</div>
    ...
    try {
        ...
        const data = await response.json();
        const comparisons = Array.isArray(data) ? data : (Array.isArray(data.deals) ? data.deals : (Array.isArray(data.comparisons) ? data.comparisons : []));

        if (!comparisons.length) {
            content.innerHTML = `<div class="text-center py-8"><div class="text-3xl mb-2">🔎</div><div class="font-semibold text-on-surface">No other store prices found</div></div>`;
            return;
        }
        ...
        content.innerHTML = `<div class="text-center py-8 text-on-surface-variant"><div class="font-semibold text-on-surface">Comparison is not available yet</div></div>`;
    }
}
...
function initBurgerMenu() {
    ...
    if (!btn || document.getElementById("customBurgerMenu")) return;
        ...
        <div class="absolute right-0 top-0 h-full w-[280px] bg-surface-container-lowest shadow-2xl p-5 flex flex-col justify-between transform translate-x-full transition-transform duration-300">
            <div>
                ...
                <div class="flex flex-col gap-2 pt-4">
                    <a href="index.html" class="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-surface-container-low text-on-surface font-label-md text-[14px]"><span class="material-symbols-outlined text-[20px] text-primary">home</span>Home</a>
                    <a href="#market-deals" onclick="closeMenu()" class="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-surface-container-low text-on-surface font-label-md text-[14px]"><span class="material-symbols-outlined text-[20px] text-primary">local_offer</span>Top Deals</a>
                    <a href="#savings-tool" onclick="closeMenu()" class="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-surface-container-low text-on-surface font-label-md text-[14px]"><span class="material-symbols-outlined text-[20px] text-primary">calculate</span>Savings Checker</a>
                </div>
            </div>
            <div class="pt-4 border-t border-surface-container text-center text-[12px] text-on-surface-variant">CheckerDiscount</div>
        </div>
    `;
...
function shareDeal(title, url) {
    ...
    if (navigator.share) {
        ...
        navigator.clipboard.writeText(url).then(() => { alert("Deal link copied to clipboard!"); }).catch(() => { alert("Deal link: " + url); });
    } else {
        alert("Deal link: " + url);
    }
}
...
function runPriceHistoryCheck() {
    ...
    const value = input?.value?.trim();
    if (!value) { alert("Please enter a product name."); return; }
    alert(`Price history check for "${value}" is ready.`);
}

function runCouponChecker() {
    const store = document.getElementById("couponStoreInput")?.value?.trim();
    if (!store) { alert("Please enter a store."); return; }
    alert("Coupon checker is ready.");
}
