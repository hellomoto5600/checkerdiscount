...
function normalizeDeal(deal) {
    ...
    // rating handling fix
    copy.rating = copy.rating || copy.store_rating || "4.8";
...
function renderCountryCategories() {
    ...
    let html = `
        <div class="flex items-center gap-2 overflow-x-auto pb-1">
            <button type="button" data-category="all" class="cd-category-btn flex-shrink-0 px-3.5 py-1.5 rounded-xl font-label-md text-[13px]">
    ...
    categories.forEach(category => {
        ...
        html += `
            <button type="button" data-category="${escapeAttribute(category)}" class="cd-category-btn flex-shrink-0 px-3.5 py-1.5 rounded-xl bg-surface-container hover:bg-surface-container-high text-on-surface-variant font-label-md text-[13px]">
...
function loadDeals(filter = "all") {
    ...
    deals.forEach(deal => {
        ...
        const dealRating = deal.rating || "4.8";
        ...
        card.innerHTML = `
            <div class="flex items-center justify-between gap-2">
                ...
                <div class="flex items-center gap-2 flex-shrink-0">
                    <span class="inline-flex items-center gap-1 text-[11px] font-bold text-on-surface bg-surface-container-low px-2 py-0.5 rounded-lg">
                        <span class="material-symbols-outlined text-[12px] text-warning-amber fill-1">star</span>
                        ${escapeHtml(String(dealRating))}
                    </span>
        ...
        <div class="absolute right-0 top-0 h-full w-[280px] bg-surface-container-lowest shadow-2xl p-5 flex flex-col justify-between transform translate-x-full transition-transform duration-300">
            <div>
                <div class="flex items-center justify-between pb-4 border-b border-surface-container">
                    ...
                    <button id="closeBurgerMenu" class="w-8 h-8 rounded-full bg-surface-container-low flex items-center justify-center text-on-surface">
                        <span class="material-symbols-outlined text-[18px]">close</span>
                    </button>
                </div>
                <div class="flex flex-col gap-2 pt-4">
                    <a href="index.html" class="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-surface-container-low text-on-surface text-[14px]">Home</a>
                    <a href="#market-deals" onclick="closeMenu()" class="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-surface-container-low text-on-surface text-[14px]">Top Deals</a>
                </div>
            </div>
        </div>
    `;
...
function shareDeal(title, url) {
    ...
    if (navigator.share) {
        ...
        navigator.clipboard.writeText(url).then(() => alert("Deal link copied to clipboard!"));
    } else {
        alert("Deal link: " + url);
    }
}
